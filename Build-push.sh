...
./gradlew --no-daemon -PenableCrossCompilerPlugin=true kayenta-web:installDist -x test
   ...
   IMAGENAME="quay.io/opsmxpublic/ubi8-spin-kayenta:${GITHASH}-${BUILD_NUMBER}"
   
   # Assigning Rhel Image Name according to Docker.io Details
   RELEASE_IMAGENAME="opsmx11/ubi8-spin-kayenta:${GITHASH}-${BUILD_NUMBER}"
   ...
   echo "Kayenta: ${IMAGENAME}"

# Quay Image Name as Artifact
echo \"Kayenta\": \"${IMAGENAME}\" > file.properties;
